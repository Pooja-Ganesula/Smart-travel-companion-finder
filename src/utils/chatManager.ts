...
export class ChatManager {
  ...
  private simulateMessageDelivery(message: ChatMessage, participants: string[]): void {
    ...
    console.log(`📨 Message delivered to chat ${message.chatId}:`);
    console.log(`   From: ${message.senderId}`);
    console.log(`   To: ${participants.filter(p => p !== message.senderId).join(', ')}`);
...
export const useChat = () => {
  ...
  const createChat = async (
    ...
    return await chatManager.createChat(participants, type, matchId, groupId);
  };
