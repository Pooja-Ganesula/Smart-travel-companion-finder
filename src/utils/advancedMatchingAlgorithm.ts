...
import type { Match, MatchSummary, Trip, User, CompatibilityScore, MatchingEngineResult, Group } from '../types';
...
const suggestGroups = (matches: Match[], currentTrip: Trip): Group[] => {
  const groups: Group[] = [];
  const highScoreMatches = matches.filter(m => m.score >= 70);

  if (highScoreMatches.length >= 2) {
    const group: Group = {
      groupId: `group-${currentTrip.tripId}`,
      tripId: currentTrip.tripId,
      name: `${currentTrip.destination} Adventure Group`,
      members: highScoreMatches.slice(0, 4).map(m => m.user),
      createdBy: 'system',
      createdAt: new Date().toISOString(),
      status: 'Planning',
      maxMembers: 6,
      groupType: currentTrip.travelType as Group['groupType'],
      destination: currentTrip.destination,
      startDate: currentTrip.startDate,
      endDate: currentTrip.endDate,
      budget: currentTrip.budget,
      description: `Automatically suggested group for ${currentTrip.destination} trip`,
    };
    groups.push(group);
  }
...
export const runAdvancedMatchingPipeline = (
  ...
  const matches = filteredCandidates
    .map(({ candidate, trip }) => {
      ...
      return {
        ...
        chatEnabled: compatibilityScore.overall >= 60,
        createdAt: new Date().toISOString(),
      } satisfies Match;
    })
  ...
  const groups = suggestGroups(matches, currentTrip);
