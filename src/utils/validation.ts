import type { User, Trip, TravelProfile, Review, EmergencyAlert, Group } from '../types';
...
export class DataValidator {
  ...
  // Group Validation
  static validateGroup(group: Partial<Group>): ValidationError[] {
    const errors: ValidationError[] = [];

    if (!group.name || group.name.trim().length < 3) {
      errors.push(new ValidationError('Group name must be at least 3 characters long', 'name', 'INVALID_GROUP_NAME'));
    }

    if (group.name && group.name.length > 50) {
      errors.push(new ValidationError('Group name must be less than 50 characters', 'name', 'GROUP_NAME_TOO_LONG'));
    }

    if (!group.destination || group.destination.trim().length < 2) {
      errors.push(new ValidationError('Destination is required', 'destination', 'INVALID_DESTINATION'));
    }

    if (!group.maxMembers || group.maxMembers < 2 || group.maxMembers > 20) {
      errors.push(new ValidationError('Group size must be between 2 and 20 members', 'maxMembers', 'INVALID_GROUP_SIZE'));
    }

    if (!group.groupType || !['Adventure', 'Leisure', 'Backpacking', 'Business'].includes(group.groupType)) {
      errors.push(new ValidationError('Valid group type is required', 'groupType', 'INVALID_GROUP_TYPE'));
    }

    if (!group.startDate || !this.isValidDate(group.startDate)) {
      errors.push(new ValidationError('Valid start date is required', 'startDate', 'INVALID_START_DATE'));
    }

    if (!group.endDate || !this.isValidDate(group.endDate)) {
      errors.push(new ValidationError('Valid end date is required', 'endDate', 'INVALID_END_DATE'));
    }

    if (group.startDate && group.endDate && new Date(group.startDate) >= new Date(group.endDate)) {
      errors.push(new ValidationError('End date must be after start date', 'endDate', 'INVALID_DATE_RANGE'));
    }

    if (group.description && group.description.length > 500) {
      errors.push(new ValidationError('Description must be less than 500 characters', 'description', 'INVALID_DESCRIPTION'));
    }
...
export const useValidation = () => {
  ...
  const validateGroup = (group: Partial<Group>) => {
    return DataValidator.validateGroup(group);
  };
