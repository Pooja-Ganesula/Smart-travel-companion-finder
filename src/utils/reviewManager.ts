...
export class ReviewManager {
  ...
  async createReview(
    ...
    const existingReview = this.reviews.find(review => 
      ...
      (review.matchId === matchId || review.groupId === groupId)
    );

    if (existingReview) {
      throw new Error('You have already reviewed this user for this match/group');
    }
  ...
  getGroupReviews(groupId: string): Review[] {
    return this.reviews
      .filter(r => r.groupId === groupId && r.isPublic)
      .sort((a, b) => b.helpfulVotes - a.helpfulVotes);
  }
...
export const useReview = () => {
  ...
  const canReview = (
    ...
    return reviewManager.canReview(reviewerId, revieweeId, matchId, groupId);
  };
